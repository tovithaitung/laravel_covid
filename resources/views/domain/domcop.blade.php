...
?>
@php 
$list_show = array('domain','dr','anchor','moz_da','RDomain_detail','maj_cf','maj_tf','price');
$list_key = array_keys((array)$list[0]);
//print_r($list_key);
@endphp
<div class="" id="app">
  <div class="row">
        ...
        <div class="col-12">
          <div class="c-card">
            ...
            <form action="/" method="GET">
                ...
                @endif
                <div class="row u-mb-medium">
                      @foreach($list_key as $key => $tmp)
                      @if($tmp != 'id' && $tmp != 'domain')
                      <div class="col-lg-12 u-mb-xsmall">
                            <div class="c-field">
                                
                                <label class="c-field__label" for="input1">{{$tmp}}</label>
                                <input class="c-input" name="{{$tmp}}"  max="100" min="-1" value="{{request()->input($tmp) ? request()->input($tmp) : -1}}" type="range" id="range_{{$key}}" oninput="range_out_{{$key}}.value=range_{{$key}}.value"><output  id="range_out_{{$key}}">{{request()->input($tmp) ? request()->input($tmp) : -1}}</output>
                            </div>
                      </div>
                      @endif
                      @endforeach
  ...
  <div class="row">
    <div class="col-12">
        ...
        <table class="c-table hide-default" id="tableCore" cellpadding="0" cellspacing="0">
          <thead class="c-table__head">
            <tr class="c-table__row">
              ...
              @foreach($list_show as $tmp)
              @if($tmp != 'id')
              @if(in_array($tmp, $list_show))
              <?php if($tmp == 'RDomain_detail') $tmp = 'Rdomain'; ?>
              <th class="c-table__cell c-table__cell--head"><a href="/?page={{(request()->input('page')) ? request()->input('page') : 1}}{{$url}}&sort={{$tmp}}{{(request()->input('type') == 'desc' && request()->input('type')) ? '&type=asc' : '&type=desc'}}" title="">{{$tmp}}</a></th>
              @endif
              @endif
              @endforeach
              @foreach($list_key as $tmp)
              @if($tmp != 'id')
              @if(!in_array($tmp, $list_show))
              <th class="c-table__cell c-table__cell--head"><a href="/?page={{(request()->input('page')) ? request()->input('page') : 1}}{{$url}}&sort={{$tmp}}{{(request()->input('type') == 'desc' && request()->input('type')) ? '&type=asc' : '&type=desc'}}" title="">{{$tmp}}</a></th>
              @endif
          ...
          <?php foreach ($list as $item) { ?>
            <tr class="c-table__row" id="row-{{ $item->id}}">
              @foreach($list_show as $tmp)
              @if($tmp != 'id')
              @if(in_array($tmp, $list_show))
              @if($tmp == 'anchor')
              <td class="c-table__cell popup" onmouseover="showAnchors({{$item->id}})" onmouseout="closeAnchors({{$item->id}})"> 
                <?php echo number_format($item->TotalAnchor) ?>
                 <span class="popuptext" id="myPopup-{{$item->id}}">
                  <p style="color:white">Anchors: <?php echo $item->domain ?> - Total: <?php echo $item->TotalAnchor ?></p> 
                  <table>
                    <thead>
                      <tr>
                        <th>Anchor</th>
              ...
              @elseif($tmp == 'RDomain_detail')
              <td class="c-table__cell popup" onmouseover="showDomain({{$item->id}})" onmouseout="closeDomain({{$item->id}})" >
                <?php $domains = json_decode($item->RDomain_detail,true); ?>
                  <span style="display: block; margin-top: 15px;"><?php echo number_format(count($domains)) ?></span>

                <span class="popuptext" id="domain-{{$item->id}}">
                  
                  <p style="color:white"><?php echo $item->domain ?> - Referrings Domain - Total: <?php echo count($domains) ?></p>
                  <table>
                    <thead>
                      <tr>
                        <th>STT</th>
              ...
              @else
              <td class="c-table__cell" ><?php echo $item->{$tmp} ?></td>
              @endif
              @endif
              @endif
              @endforeach
              ...
              <td class="c-table__cell popup" onmouseover="showDomain({{$item->id}})" onmouseout="closeDomain({{$item->id}})" >
                ...
                <span class="popuptext" id="domain-{{$item->id}}">
                  <p style="color:white"><?php echo $item->domain ?> - Referrings Domain - Total: <?php echo $item->Rdomain ?></p>
                  <table>
                    <thead>
                      <tr>
                        <th>STT</th>
      ...
      <table class="c-table" id="tableCustom" style="display: none" v-show="active">
          ...
          <tbody>
          <tr class="c-table__row" :id="'row-' + element.id" v-for="element in elements">
              <td class="c-table__cell" v-html="element.domain_name" ></td>
              <td class="c-table__cell" v-html="element.dr"></td>
              <td class="c-table__cell popup" @mouseover="showDomain(element.id)" @onmouseout="closeDomain(element.id)" >@{{ element.RDomain }}
                  <span class="popuptext" :id="'domain-' + element.id">
              ...
              <td class="c-table__cell popup" @mouseover="showAnchors(element.id)" @mouseout="closeAnchors(element.id)" >@{{ element.TotalAnchor }}
                  <span class="popuptext" :id="'myPopup-' + element.id">
              ...
              <td class="c-table__cell">
                  <div class="form-group box">
                      <select class="form-control" :id="'select1-' + element.id" @change="changeStatus(element.id)">
                          <option :value="element.id + '-0'"  >All</option>
                          <option :value="element.id + '-1'"  >Seen</option>
                          @if(Auth::user()->email == 'admin@domain.com')
                          <option :value="element.id + '-2'"  >Bought</option>
                          <option :value="element.id + '-3'"  >Interested</option>
                          @endif
                          <option :value="element.id + '-4'"  >Buy</option>
                          <option :value="element.id + '-5'"  >SEO</option>
                          <option :value="element.id + '-6'"  >PBN</option>
                          <option :value="element.id + '-7'"  >PBN</option>
                      </select>
                  </div>
              </td>
          </tr>
